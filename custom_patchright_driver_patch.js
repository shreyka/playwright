...
// server/frames.ts  
// ----------------------------
// MODIFIED TO PRESERVE COMPATIBILITY
// Keeping essential infrastructure but using safer element selection
...
// SAFE VERSION: Use original Playwright logic instead of complex patchright logic
...
// });
// retryWithProgressIfNotConnectedMethod.setBodyText(`// Use standard Playwright element selection for better compatibility
...
//   const elementsAmount = await result.getProperty("length");
//   const amount = await elementsAmount.jsonValue();
...
//   const elementHandle = await result.evaluateHandle((elements, index) => elements[0]);
  
//   if (amount > 1) {
//     if (resolved.info.strict) {
//       await resolved.injected.evaluateHandle((injected, { info, elements }) => {
//         throw injected.strictModeViolationError(info.parsed, elements);
//       }, { info: resolved.info, elements: [elementHandle] });
//     }
//     progress.log("  locator resolved to " + amount + " elements. Proceeding with the first one.");
//   }

//   try {
//     const actionResult = await action(elementHandle);
//     if (actionResult === 'error:notconnected') {
//       progress.log('element was detached from the DOM, retrying');
//       return continuePolling;
//     } else if (actionResult === 'internal:continuepolling') {
//       return continuePolling;
//     }
//     return actionResult;
//   } finally { }
...
//   let visible = false;
//   if (attached) {
//     visible = await handle.evaluateInUtility(([injected, node]) => {
//       return injected.utils.isElementVisible(node);
//     });
//   }
...
//         const state = injected.elementState(node, 'visible');
...
// return await controller.run(async progress => {
//   progress.log("waiting for " + this._asLocator(selector));
//   const promise = await this._retryWithProgressIfNotConnected(progress, selector, false, false, async handle => {
//     // Simple count using standard Playwright
//     const resolved = await this.selectors.resolveInjectedForSelector(selector, { strict: false });
//     if (!resolved) return 0;
//     const result = await resolved.injected.evaluateHandle((injected, { info }) => {
//       return injected.querySelectorAll(info.parsed, document);
//     }, { info: resolved.info });
//     const count = await result.getProperty("length");
//     return await count.jsonValue();
//   });
//   return promise;
// }, 100);
// `);
...
// const promise = await this._retryWithProgressIfNotConnected(progress, selector, !isArray, false, async handle => {
//   return await handle.evaluateInUtility(async ([injected, node, { options }]) => {
//     return await injected.expect(node, options, [node]);
//   }, { options });
// });
...
//     return await handle.evaluateInUtility(([injected, node, { callbackText, taskData }]) => {
//       const callback = injected.eval(callbackText);
//       return callback(injected, node, taskData);
//     }, { callbackText, taskData });
//   });
//   return scope ? scope._context._raceAgainstContextDestroyed(promise) : promise;
// }, this._page._timeoutSettings.timeout(options));`)

// NOTE: We intentionally do NOT add _customFindElementsByParsed method 
// as it's the source of the recorder click issues
...
// Update createPageBindingScript to match working implementation  
